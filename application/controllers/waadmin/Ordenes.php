 ...
 */
 ...
 //$data['wa_tipo'] = $tipo;
   ...
   $data['wa_menu'] = 'Ordenes';

 $sessionName = 's_ordenes'; //Session name

 //Paginacion
 $base_url = base_url() . "waadmin/ordenes/index";
 ...
 if (isset($_GET['refresh'])) {
   ...
   redirect("waadmin/ordenes/index");
 }
 ...
 $data['total_registros'] = $this->Ordenes->total_registros($post);

 //Listado
 $data['listado'] = $this->Ordenes->listado($per_page, $page, $post);
 ...
 $this->template->title('Ordenes');
 $this->template->build('waadmin/ordenes/index', $data);
}
...
function editar($tipo='C',$id=NULL){
 ...
 $data['back_url'] = base_url('waadmin/ordenes/index');
 if(isset($id)){
   $data['edit_url'] = base_url('waadmin/ordenes/editar/E/' . $id);
 }
 ...
 if($tipo == 'E' || $tipo == 'V'){
  ...
  $orden_info = $this->Ordenes->get_row($data_row);
  $data['post'] = $orden_info;

  /**
   * Detalles de la orden
   */
  $data_detalles = array("orden_id" => $orden_info['id']);
  $data['productos'] = $this->Ordenes->get_detalles($data_detalles);
}
...
$this->template->title($data['tipo'] . ' Orden');
$this->template->build('waadmin/ordenes/editar', $data);
}
